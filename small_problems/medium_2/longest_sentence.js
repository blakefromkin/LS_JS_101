...
ALGORITHM
  - Set punctuation = an array with all the sentence enders
  - Set an array = the string split by spaces
  ...
  - For each word in words
    - If the word includes ^punctuation
      -  if longest < (index - sentenceStart)
        - set longest = index - sentenceStart (otherwise, keep it longest)
        - set longestIdx = sentenceStart
      - set sentenceStart to index + 1

  - let sentence = substring from longestIdx through longest
...
function longestSentence(string) {
  ...
  words.forEach((word, index) => {
    PUNCTUATION.forEach(char => {
      if (word.includes(char)) { // If end of sentence
        let length = (index + 1) - sentenceStart;
        if (longest < length) { // Check if new longest
          longest = length;
          longestIdx = sentenceStart;
        }
  ...
  console.log(words.splice(longestIdx, longest).join(' '));
  console.log(`The longest sentence has ${longest} words.`);
  console.log('');
}
